...
package :mysql, :provides => :database do
  ...
  apt %w( mysql-server mysql-client libmysqlclient-dev ) do
    ...
    post :install, "echo 'CREATE DATABASE IF NOT EXISTS rails; GRANT ALL PRIVILEGES ON rails.* TO rails@localhost IDENTIFIED BY \"#{DB_PASSWORD}\";'|mysql -u root -p#{ROOT_PASSWORD}"
  end
  ...
  verify do
    ...
    @commands << "mysqladmin -u rails -p#{DB_PASSWORD} ping"
  end
